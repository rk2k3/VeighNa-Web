...
vt_symbols = ["COPPER.NYMEX", "SILVER.NYMEX", "URANIUM.NYMEX"]
...
df_result.to_csv("results_portfolio_combined.csv")

statistics = engine.calculate_statistics(output=False)
with open("results_portfolio_combined_stats.json", "w") as f:
    json.dump(statistics, f, indent=2, default=str)

print("\n=== Portfolio (40% Copper / 30% Silver / 30% Uranium) ===")
for key, value in statistics.items():
    print(f"{key}: {value}")

fig, axes = plt.subplots(3, 1, figsize=(12, 10))
axes[0].plot(df_result.index, df_result["balance"], color="#2E75B6")
axes[0].set_title("Portfolio Balance Curve")
...
plt.tight_layout()
plt.savefig("results_portfolio_combined_chart.png", dpi=150)
print("\nChart saved to results_portfolio_combined_chart.png")

...
class PortfolioMinVarStrategy(StrategyTemplate):
    ...
    parameters = ["est_win", "w_max", "rebalance_days"]
    variables = ["bar_count"]
    ...
    def __init__(
        ...
        self.closes: Dict[str, dict] = {
            vt_symbol: {} for vt_symbol in vt_symbols
        }
        self.last_date = None
    ...
    def _compute_weights(self) -> Dict[str, float]:
        ...
        if len(common_dates) < needed:
            self.write_log(
                "Insufficient history for MinVar - using equal weights"
            )
            return equal
        ...
        if not result.success:
            self.write_log(
                f"MinVar solver failed ({result.message}) - using equal weights"
            )
            return equal
        ...
        self.write_log(
            f"Portfolio variance = {portfolio_variance(weights):.8f}"
        )
